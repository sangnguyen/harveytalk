...
- (NSString *)sendPingToServer:(XMPPJID*)myJid
{
	// This is a public method.
	// It may be invoked on any thread/queue.
	
	return [self sendPingToServerWithTimeout:DEFAULT_TIMEOUT myJid:myJid];
}
...
- (NSString *)sendPingToServerWithTimeout:(NSTimeInterval)timeout myJid:(XMPPJID*)myJid
{
	// This is a public method.
	// It may be invoked on any thread/queue.
	...
	XMPPIQ *iq = [XMPPIQ iqWithType:@"get" from:myJid to:nil elementID:pingID child:ping];
	DLog(@"iq:  %@",iq);
	[xmppStream sendElement:iq];
