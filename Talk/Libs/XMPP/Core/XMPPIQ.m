...
+ (XMPPIQ *)iqWithType:(NSString *)type from:(XMPPJID*)myJid to:(XMPPJID *)jid elementID:(NSString *)eid child:(NSXMLElement *)childElement
{
	return [[[XMPPIQ alloc] initWithType:type from:myJid to:jid elementID:eid child:childElement] autorelease];
...
- (id)initWithType:(NSString *)type from:(XMPPJID*)myJid to:(XMPPJID *)jid elementID:(NSString *)eid child:(NSXMLElement *)childElement
{
	if ((self = [super initWithName:@"iq"]))
	{
		if (type)
        ...
        if(myJid)
            [self addAttributeWithName:@"from" stringValue:[myJid full]];
		if (jid)
			[self addAttributeWithName:@"to" stringValue:[jid full]];
